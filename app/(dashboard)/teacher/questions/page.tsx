...
export default function TeacherQuestionsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">Student Questions</h1>
      ...
      {loading ? (
        ...
        <Card className="border border-amber-900/15 bg-stone-900 shadow-md">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-stone-400">No student questions yet.</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-500">{new Date(question.createdAt).toLocaleDateString()}</p>
                  </div>
                  {question.visibility === "PUBLIC" ? (
                    <Badge className="bg-emerald-900/30 text-emerald-400 border border-emerald-700/30 ml-auto"><Globe className="w-3 h-3 mr-1" />Public</Badge>
                  ) : (
                    <Badge className="bg-amber-900/30 text-amber-400 border border-amber-700/30 ml-auto"><Lock className="w-3 h-3 mr-1" />Private</Badge>
                  )}
                </div>
              </CardHeader>
                    ...
                    {question.answers.map((answer) => (
                      <div key={answer.id} className="bg-stone-800/50 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Badge variant="outline" className="text-xs border-amber-900/20 text-stone-400">{answer.answerer.role}</Badge>
                        </div>
